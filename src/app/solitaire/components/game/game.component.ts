...
export class GameComponent implements OnInit {
  ...
  private puntajeGuardado: boolean = false;
  ...
  async solveWithAnimation(from: SolveFrom, to: SolveTo | null = null): Promise<void> {
    ...
    if (this.isGameEnded && !this.puntajeGuardado) 
    {
      await this.puntajeService.guardarPuntos("solitario", this.movements);
      Swal.fire({
        icon: 'info',
        title: 'Juego Terminado',
        html: `
          <div style="font-size: 1.2rem; margin-bottom: 1rem;">
            Cantidad de Movimientos: <strong>${this.movements}</strong>
          </div>
